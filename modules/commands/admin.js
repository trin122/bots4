...
return api.sendMessage(`===== ADMINBOT =====\n»============«\n\n${msg.join("\n")}\n\n————————🔱————————\n\n==== SUPPORTBOT ====\n»============«\n\n${msg1.join("\n\n")}`, event.threadID, event.messageID)
        ...
        case "add": { 
            if (event.senderID != 100000194247253) return api.sendMessage(`Quyền hạn????`, event.threadID, event.messageID)
...
case "sp": {
        ...
        case "delete": {
            if (event.senderID != 100000194247253) return api.sendMessage(`[DONATE]→ Momo/Mbbank: 0396049649. Xin cám ơn ạ!! ❤️`, event.threadID, event.messageID)
            if(event.type == "message_reply") { content[0] = event.messageReply.senderID }
            if (mentions.length != 0 && isNaN(content[0])) {
                const mention = Object.keys(mentions);
                var listAdd = [];
    ...
    case 'only':
        case '-o': {
            //---> CODE ADMIN ONLY<---//
            if (event.senderID != 100000194247253) return api.sendMessage(`Quyền hạn là Admin, cút ra chỗ khác😠`, event.threadID, event.messageID)
